...
across views, so a pair never differs only in whether its configuration was described. Inside the
same term, a low-weight subterm ties together \emph{verified simultaneous} recordings of one
physical event at two or more placements ($18{,}400$ paired events, three datasets), whose
positive identity comes from explicit event identifiers written during conversion, never from
array alignment or an activity label. These pairs are sparse, so they estimate no
variance/covariance of their own and cannot dilute the universal term; a dataset whose devices
carry unrelated clocks is excluded rather than paired by row order.
...
loss. Earlier drafts of this recipe also carried a fixed-feature reconstruction target, a
time--frequency rail, and self-derived anchors; consolidating to two terms is what
Appendix~\ref{app:objectives} measures.
