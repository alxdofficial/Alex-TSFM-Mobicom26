...
acquisition statistics. Phase A learns from masked contextual prediction and consistency across
...
\begin{itemize}
  ...
  \item \textbf{We build a leakage-aware enrollment protocol} in which support and query come from
        ...
        frozen features.
  \item \textbf{We state what the protocol must contain before it can test the claim.} A two-sided
        admissibility score collapses to a per-candidate constant whenever the enrolled and queried
        configurations coincide, so cross-configuration cells are a precondition for measuring the
        mechanism rather than an extension. We keep the seven-dataset test roster sealed.
\end{itemize}
