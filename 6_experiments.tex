...
Appendix~\ref{app:corpus} plots the corpus by placement and rate and records the six datasets
...
material---harnet is pretrained on roughly $700$k person-days against our $2{,}858$
...
by the $93$-label training vocabulary, while four are not. A method that only needs to name
\emph{sitting}, \emph{standing} and \emph{walking} should do equally well on both groups; a
closed-vocabulary method should not.
...
UT-Complex asks for \emph{drinking coffee}, \emph{eating} and \emph{smoking}---three
wrist-mounted activities a closed sit/stand/walk vocabulary cannot express at all---and
InclusiveHAR distinguishes \emph{ramp ascent} from \emph{walking upstairs}. Where a held-out set
\emph{is} covered by the training vocabulary, we say so rather than counting it as evidence for
an open-set claim it does not test.
