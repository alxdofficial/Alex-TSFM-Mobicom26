...
than implying a guarantee we cannot honour.
...
regime of its own; we will report that outcome plainly. Table~\ref{tab:per_dataset} gives the
per-dataset breakdown behind this curve, including InclusiveHAR stratified by ability.
...
The live recipe is two terms, so the ablation is short: strip the masked-latent objective, then
the cross-placement subterm, then swap the tokenizer's front end and its second resolution
(Appendix~\ref{app:objectives}). One control matters more than the ablation itself. The
...
beat its own initialisation is reported as such---a bar that some arms of this system have
already failed.
