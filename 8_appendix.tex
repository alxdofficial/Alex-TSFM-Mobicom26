...
$4.2\mathrm{e}{-}4$ with $1$k warmup, weight decay $0.05$, gradient clip $1.0$. Both objective
weights are $1.0$; the cross-placement subterm is weighted $0.1$ within the relation term, and
...
acquisition configurations. A fifth of each batch is reserved for verified simultaneous pairs.
Positive identity comes from explicit physical-event identifiers written during conversion and
carried into the pretraining index, never inferred from equal array lengths or row alignment, and
never from an activity label.
...
\begin{table}[htbp]
  \centering
  \caption{Phase-A ablation, macro-F1 over the held-out datasets. R0--R2 strip the live recipe
  back one term at a time; the tokenizer rows test whether the physical-Hz front end and the two
  resolutions earn their place; the last row is a control, not a variant.}
  ...
  \begin{tabular}{l cc}
    ...
    \midrule
    R2: masked latent $+$ relation $+$ cross-placement (live) & \PH & \PH \\
    \quad R1: $-$ masked latent (relation only)      & \PH & \PH \\
    \quad R0: $-$ cross-placement (augmentation relation only) & \PH & \PH \\
    \midrule
